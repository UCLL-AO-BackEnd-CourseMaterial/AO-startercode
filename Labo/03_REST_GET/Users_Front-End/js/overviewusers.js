  ...
  if (users.length) {
    users
      .forEach((user) => {
        ...
        addTableCell({ tableRow, value: user.age });
        addTableRow({
  ...
  await (async () => {
    ...
    if (adults === "yes") {
      ...
      response = await fetch(
        "http://localhost:8080/api/users/adults"
      );
    }
    else if (minAge.value.length && maxAge.value.length) {
      response = await fetch(
        "http://localhost:8080/api/users/search/age/" + minAge.value + "/" + maxAge.value)
    } else {
      caption.innerHTML = ""
    ...
    users.length = 0
    users.push(...result)
    responseOldest = await fetch("http://localhost:8080/api/users/oldest")
...
const addEventListeners = () => {
  filterByAdults.addEventListener("click", () => {
    adults = "yes"
    fetchAndRenderUsers()
  })
